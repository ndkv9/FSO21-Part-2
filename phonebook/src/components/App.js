...
function App() {
  ...
  useEffect(() => {
    contactService
      .getAll()
      .then(returnedPeople => {
        returnedPeople.map(person => console.log(`${person.name} ${person.id}`))
  ...
  const onSubmit = event => {
    ...
    if (nameArray.includes(newName.toLowerCase())) {
      ...
      if (confirmation) {
        const selectedPerson = people.find(person => person.name === newName)
        const newContact = {
        ...
        contactService
          ...
          .then(returnedContact => {
            ...
            setPeople(
              people.map(person =>
                person.id !== selectedPerson.id ? person : returnedContact
              )
            )
          })
          .catch(err => {
      ...
      contactService
        .create(newPerson)
        .then(returnedPerson => {
          setSuccessfulMessage({contact: returnedPerson.name, action: 'Added'})
          setTimeout(
            () => setSuccessfulMessage({contact: null, action: ''}),
            3000
          )
  ...
  const removeContact = id => {
    contactService
      .remove(id)
      .then(returnedContact => {
        setSuccessfulMessage({contact: returnedContact.name, action: 'Deleted'})
        setTimeout(
          () => setSuccessfulMessage({contact: null, action: ''}),
          3000
        )
  ...
  return (
    <React.Fragment>
      ...
      <h2>Numbers</h2>
      <People people={people} regexp={regexp} remove={removeContact} />
    </React.Fragment>
  )
}
